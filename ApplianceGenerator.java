...
public class ApplianceGenerator {
    ...
    public ApplianceGenerator(String n, int on, int off, double pOn, boolean s, double pSmart) {
        name = n;
        onW = on;
        offW = off;
        probOn = pOn;
    ...
    public static void main(String[] args) throws IOException {
        ApplianceGenerator[] app = new ApplianceGenerator[100];  // default 100 possible appliance types
        File inputFile = new File("ApplianceDetail.txt");
        Scanner scan = new Scanner(inputFile);
        int count = 0;
        while (scan.hasNext()) {
            StringTokenizer stringToken = new StringTokenizer(scan.nextLine());
            app[count] = new ApplianceGenerator(stringToken.nextToken(","),
